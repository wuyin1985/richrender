...
impl Interpolate for Quat {
    ...
    fn cubic_spline(
        ...
        let t = amount;
        let p0 = source[1];
        let m0 = source[2] * (target_time - source_time);
        let p1 = target[1];
        let m1 = target[0] * (target_time - source_time);

        let result = p0 * (2.0 * t * t * t - 3.0 * t * t + 1.0)
            + m0 * (t * t * t - 2.0 * t * t + t)
...
#[derive(Debug, Clone)]
struct Sampler<T> where T: Clone {
    interpolation: Interpolation,
    times: Vec<f32>,
    values: Vec<T>,
}

impl<T> Sampler<T> where T: Clone {
...
#[derive(Debug, Clone)]
struct Channel<T> where T: Clone {
    sampler: Sampler<T>,
    node_index: usize,
}
...
impl Animation {
    ...
    pub fn animate(&mut self, nodes: &mut Nodes, time: f32) -> bool {
        ...
        translations.iter().for_each(|(node_index, translation)| {
            nodes.nodes_mut()[*node_index].set_local_position(*translation);
        });
        rotations.iter().for_each(|(node_index, rotation)| {
            nodes.nodes_mut()[*node_index].set_local_rotation(*rotation);
        });
        scale.iter().for_each(|(node_index, scale)| {
            nodes.nodes_mut()[*node_index].set_local_scale(*scale);
        });
